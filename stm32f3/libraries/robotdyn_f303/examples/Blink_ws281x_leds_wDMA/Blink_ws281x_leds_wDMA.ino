...
void loop() {
  ...
  for (unsigned indx=0; indx < sizeofs(frames); ++indx) {
    ...
    // kick off the timer, transfer a frame of color pulses
    dma_start();

    // wait for DMA completion flag or abort after 100msec timeout
    // normally, this should take ~120 usecs for 4 leds
...
void on_dma_complete()
{
  ...
  wiringEx::digitalWriteEx(LED_BUILTIN,(wiringEx::digitalReadEx(LED_BUILTIN) ? LOW : HIGH));
}
